...
for i, p in enumerate(doc.paragraphs):
    ...
    if t == '实习单位:':
        p.clear()
        run = p.add_run('实习单位:成都信息工程大学')
        run.font.name = 'Times New Roman'
        run.font.size = Pt(14)
        run.element.rPr.rFonts.set(qn('w:eastAsia'), '宋体')
    elif t.startswith('实习时间'):
        p.clear()
        run = p.add_run('实习时间：2026年7月2日至2026年7月14日')
        run.font.name = 'Times New Roman'
        run.font.size = Pt(14)
        run.element.rPr.rFonts.set(qn('w:eastAsia'), '宋体')
    elif t == '学生姓名:               学号:':
...
# 设置落款日期
for i, p in enumerate(doc.paragraphs):
    if p.text.strip() == '年    月    日':
        p.clear()
        run = p.add_run('2026年8月    日')
        run.font.name = 'Times New Roman'
        run.font.size = Pt(14)
        run.element.rPr.rFonts.set(qn('w:eastAsia'), '宋体')
        break
...
for r in range(3):
    for c in range(len(table.rows[r].cells)):
        t = table.rows[r].cells[c].text.strip()
        if t == '专业班级':
            set_cell_text(table.rows[r].cells[c+1], '软件工程', Pt(10))
        elif t == '学生电话':
            set_cell_text(table.rows[r].cells[c+1], '（学生电话）', Pt(10))
        elif t == '实习单位':
            set_cell_text(table.rows[r].cells[c+1], '成都信息工程大学', Pt(10))
        elif t == '电话':
            set_cell_text(table.rows[r].cells[c+1], '（单位电话）', Pt(10))
        elif t == '通讯地址':
            set_cell_text(table.rows[r].cells[c+1], '四川省成都市西南航空港经济开发区学府路一段24号', Pt(10))
        elif t == '邮编':
            set_cell_text(table.rows[r].cells[c+1], '610225', Pt(10))
        elif t == '学生姓名':
            # 第一个是标签，跳过；第二个是值
            nc = c + 1
...
for idx, (m, d, s) in enumerate(attendance):
    ...
    row = table.rows[ri]
    if len(row.cells) > 11: set_cell_text(row.cells[11], m, Pt(9))
    if len(row.cells) > 12: set_cell_text(row.cells[12], d, Pt(9))
    if len(row.cells) > 14: set_cell_text(row.cells[14], s, Pt(9))
    ...
    for c_idx in range(min(3, len(row.cells))):
        ...
        if t in evals:
            ...
            if col and col < len(row.cells):
                run = row.cells[col].paragraphs[0].add_run('✓')
                run.font.size = Pt(12)
                run.bold = True
            break
# 综合评价
