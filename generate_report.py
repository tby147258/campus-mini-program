...
add_table(
    ...
    [
        ...
        ['MySQL', '8.0+', '关系型数据库'],
        ['Redis', '6.x+', '缓存与验证码存储'],
        ['JWT (JJWT)', '0.12.3', 'Token认证（HMAC-SHA256）'],
        ['SpringDoc OpenAPI', '2.3.0', 'API文档自动生成（Swagger UI）'],
        ['Vue 3', '3.x', '管理后台前端框架'],
        ['Element Plus', '最新版', 'UI组件库'],
        ['微信小程序', '原生框架', '学生端移动应用'],
        ['和风天气API', '免费版', '实时天气数据'],
        ['Trae IDE', 'DeepSeek-V4-Flash', 'AI辅助开发工具'],
    ])
...
add_table(
    ...
    [
        ...
        ['个人中心', '设备自动登录', '微信自动登录，deviceId持久化方案，同一设备始终同一用户'],
        ['个人中心', '个人信息', '完善和修改个人信息（昵称、学号、手机号）'],
        ['个人中心', '我的发布', '查看个人发布的失物招领'],
        ['个人中心', '退出登录', '清除登录状态，记录操作日志'],
    ]
)
...
add_bullet('安全性：密码采用BCrypt加密，认证使用JWT Token（HMAC-SHA256），关键操作需@RoleRequired管理员权限，滑块验证码防自动化攻击')
add_bullet('响应速度：接口响应时间不超过2秒，列表查询支持分页，天气数据Redis缓存减少API调用')
add_bullet('可用性：系统7×24小时可用，支持50+并发用户')
add_bullet('可维护性：代码规范清晰，数据库设计完整（7张表+22个索引），API文档自动生成（SpringDoc OpenAPI）')
add_bullet('数据一致性：关键操作使用@Transactional事务管理，逻辑删除而非物理删除')
add_bullet('配置安全：JWT密钥、数据库密码、API密钥均通过环境变量注入，禁止硬编码')
...
add_table(
    ['接口分类', '路径', '说明'],
    [
        ['认证', '/api/auth/**', '登录、注册、Token管理、个人信息更新、密码重置'],
        ['验证码', '/api/captcha/**', '滑块验证码生成与验证、邮箱验证码发送'],
        ['公告', '/api/announcement/**', '公告CRUD、分类筛选、全文搜索'],
        ['失物招领', '/api/lost-found/**', '失物信息CRUD与审核、多条件组合筛选'],
        ['报修工单', '/api/repair/**', '工单CRUD与状态机流转、工单编号自动生成'],
        ['文件上传', '/api/file/**', '单图/多图上传、格式与大小校验'],
        ['天气', '/api/weather/**', '实时天气与3天预报查询、Redis缓存'],
        ['统计', '/api/statistics', '系统运营数据统计（公告数/工单数/用户数）'],
        ['用户管理', '/api/users/**', '用户CRUD、Redis缓存管理'],
        ['系统配置', '/api/system-config/**', '配置项CRUD'],
        ['操作日志', '/api/operation-logs/**', '操作日志分页查询、按模块筛选'],
    ]
)
...
add_body('认证模块是整个系统的安全基础，采用JWT（JSON Web Token）实现无状态认证，支持微信小程序设备登录和管理员账号密码登录双模式。')
add_body('JwtConfig 类通过 @ConfigurationProperties(prefix = "jwt") 绑定配置，采用构造函数绑定模式，在校验密钥长度（≥32字节）和过期时间（>0）时启动 Fail-Fast，配置错误则项目无法启动，从源头避免安全隐患。密钥通过环境变量 JWT_SECRET 注入，生产环境与开发环境使用不同的密钥，禁止硬编码。')
add_body('JwtUtil 基于 JJWT 0.12.3 库实现 Token 的生成与解析，Token 中包含 userId（subject）和 role（自定义 claim），使用 HMAC-SHA256 算法签名。过期时间默认24小时，支持自定义过期时间。')
add_body('JwtAuthInterceptor 实现 HandlerInterceptor 接口，在 preHandle 方法中完成认证鉴权：首先匹配白名单路径放行，然后检查方法/类上的 @NoAuth 和 @RoleRequired 注解，解析 Authorization Header 中的 Bearer Token，将 userId/role 设置到 UserContext 中。在 afterCompletion 方法中清除 UserContext，防止 ThreadLocal 内存泄漏。拦截器支持区分 JWT 过期、签名错误和格式错误分别返回 401 响应，便于前端区分处理。')
...
add_body('微信小程序登录采用 deviceId 持久化方案：在 app.js 的 onLaunch 中生成唯一 deviceId 并存入本地存储，每次启动时使用该 deviceId 调用 POST /api/auth/wx-login 接口实现自动登录，同一设备始终使用同一用户标识。后端 UserServiceImpl.loginOrRegister() 使用 @Transactional(rollbackFor = Exception.class) 防止并发重复创建用户，OpenId 使用 UUID.randomUUID() 生成避免高并发重复。')
add_body('管理员登录流程：输入账号密码 → 验证滑块验证码 → 消费 passToken → 验证账号密码（BCrypt.matches）→ 签发 JWT Token → 记录操作日志。验证码 passToken 采用一次性消费策略，防止重放攻击。')
...
add_paragraph('5.2.5 滑块验证码与邮箱验证码模块实现', bold=True, size=12)
add_body('验证码模块包含滑块验证码和邮箱验证码两部分，均由 CaptchaService 实现。')
add_body('滑块验证码使用纯 Java AWT 图形绘制技术实现，无需依赖第三方验证码服务。在 280×150 像素的背景图上绘制渐变背景和随机干扰线，在随机位置（x范围30~250，y范围0~110）抠出 40×40 像素的拼图块（添加白边和半透明效果，原图挖空区域变暗），返回 base64 编码的背景图和拼图块。')
add_body('验证码的正确位置 x 坐标存入 Redis（5分钟有效），前端拖拽验证时接受容差 5 像素。验证通过后生成一次性 passToken（5分钟有效），用于后续的登录或密码重置操作，防止重放攻击。')
add_body('邮箱验证码用于管理员忘记密码场景：CaptchaService.sendEmailCode() 支持 scene 场景参数（如 "forgot-password"），防止跨场景验证码复用。验证码为6位随机数字，存入 Redis 有效期5分钟，通过控制台打印模拟邮件发送。验证时需同时校验 email、scene 和 code 三个参数，确保场景匹配。AuthController 的 forgot-password 接口还增加了 IP/邮箱级别的速率限制（60秒内同一邮箱仅允许发送1次），防止接口被滥用。')
...
add_body('RedisConfig 配置了 Jackson2JsonRedisSerializer 作为 RedisTemplate 序列化方案，不使用 activateDefaultTyping 以防止 Jackson 多态反序列化 RCE 漏洞。对于 Spring @Cacheable/@CacheEvict 注解缓存，配置了 GenericJackson2JsonRedisSerializer 作为缓存值序列化器（启用 DefaultTyping 以写入 @class 类型信息，确保反序列化为正确 Java 类型），同时设置了缓存 TTL 和禁止缓存 null 值。')
add_body('UserServiceImpl 中，getById 方法使用 @Cacheable(value = "user", key = "#id") 缓存用户查询结果，缓存前将密码字段置空防止泄露；updateById 和 removeById 使用 @CacheEvict(value = "user", key = "#id") 在数据变更后清除缓存，保证数据一致性。')
add_body('和风天气 API 查询结果也通过 Redis 缓存，减少第三方 API 调用频率。CaptchaService 使用 Redis 存储验证码位置、邮箱验证码和 passToken，利用 Redis 的过期机制自动清理过期数据。')

add_paragraph('5.2.8 天气查询模块实现', bold=True, size=12)
add_body('天气查询模块通过 QWeatherClient 对接和风天气 API，提供实时天气查询和 3 天天气预报功能。')
add_body('QWeatherClient 使用 RestTemplate 发起 HTTP 请求，支持 gzip 压缩响应解压，通过检测 GZIP 魔数（0x1F 0x8B）自动判断是否需要解压。自定义错误处理器，不因 HTTP 4xx/5xx 状态码抛异常，通过检查响应体中的业务 code 判断请求是否成功。')
add_body('配置文件通过 QWeatherProperties 绑定，host 和 geo-host 支持免费订阅和付费订阅两种模式，城市搜索使用独立的地理编码 API 域名。API 密钥通过环境变量 QWEATHER_KEY 注入，避免明文提交到仓库。')
add_body('WeatherController 提供两个接口：实时天气查询（GET /api/weather/now）和 3 天预报查询（GET /api/weather/3d），均支持按城市名称或 LocationID 查询，查询结果通过 Redis 缓存减少第三方 API 调用。')
...
add_body('请求封装在 utils/request.js 中，统一管理 baseUrl、请求头、Token 注入和错误处理。使用 loadingCount 计数器模式确保 wx.showLoading/wx.hideLoading 配对使用，避免并发请求时的加载状态混乱。同时提供 isLoadingShowing 状态标记，防止误调用 wx.hideLoading() 触发配对警告。')
add_body('登录采用设备ID持久化方案：在 app.js 的 onLaunch 中生成唯一 deviceId 并存入本地存储，每次启动时使用该 deviceId 调用 wx-login 接口实现自动登录，同一设备始终使用同一用户标识。')
add_body('个人中心页面支持用户完善个人信息（昵称、学号、手机号），通过 PUT /api/auth/profile 接口提交。退出登录时调用 POST /api/auth/logout 接口记录操作日志。')
...
add_paragraph('提示词示例4：实现滑块验证码组件', bold=True, size=11)
add_paragraph('"请实现一个 Vue 3 滑块验证码组件。需要支持：1）从后端获取验证码图片（bgImage带缺口背景图 + puzzleImage拼图块）；2）拼图块跟随滑块水平移动，垂直位置与缺口对齐；3）验证时将滑块位置转换为像素值发送给后端验证；4）验证通过后获取passToken用于后续登录。"', size=10, first_line_indent=0.74)
add_paragraph('AI响应：生成了CaptchaSlider.vue组件，包含canvas/div渲染逻辑、拖拽事件处理、位置计算与验证逻辑。', size=10, first_line_indent=0.74)

add_paragraph('提示词示例5：对接和风天气API', bold=True, size=11)
add_paragraph('"请实现一个和风天气API客户端。需要支持：1）实时天气查询和3天预报查询；2）gzip压缩响应解压；3）城市搜索功能；4）自定义错误处理，不因HTTP错误码抛异常；5）配置通过@ConfigurationProperties绑定。"', size=10, first_line_indent=0.74)
...
add_bullet('小程序POST/PUT请求500错误：AI 定位到 wx.request 的 data 参数未使用 JSON.stringify 序列化，导致后端无法解析请求体，修复后所有非GET请求统一进行 JSON 序列化')
add_bullet('CaptchaSlider滑块验证码组件问题：AI 修复了拼图块不显示（需使用 puzzleImage 叠加层）、位置匹配不准确（wrapper尺寸固定为280px与后端一致）、token传递错误等问题')
add_bullet('Vite模块导入失败：AI 发现 vite.config.js 缺少 resolve.alias 配置，导致 @ 别名无法解析，添加 path 映射后修复')
...
add_table(
    ...
    [
        ...
        ['TC-AUTH-09', '微信小程序自动登录', '返回Token和用户信息', '通过'],
        ['TC-AUTH-10', '发送重置密码验证码', '控制台打印验证码', '通过'],
        ['TC-AUTH-11', '验证码场景校验', '场景不匹配则验证失败', '通过'],
        ['TC-AUTH-12', '忘记密码速率限制', '60秒内重复请求返回429', '通过'],
        ['TC-AUTH-13', '更新个人信息', '昵称/学号/手机号更新成功', '通过'],
    ]
)
...
add_bullet('CaptchaSlider组件Bug：修复拼图块不显示、位置匹配不准确、token传递错误')
add_bullet('小程序POST/PUT请求500错误：data统一使用JSON.stringify序列化')
add_bullet('Vite模块导入失败：添加resolve.alias配置')
add_bullet('小程序退出登录后重新登录提示"登录凭证不能为空"：修复deviceId空字符串判断逻辑')
